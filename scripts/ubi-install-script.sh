...
help()
{
	[ $# -ge 1 ] && (>&2 echo $@)
	cat << EOFSYN
syntax: $0 --device MyRouter [--address 0xdeadbeef] [--arch ARM] ( --kernel kernelimage --rootfs rootfsimage ) | (--sysupgrade sysupgrade.bin) [--kernel-volname kernel] [--rootfs-volname rootfs] [--ubi-partition ubi] [--mode fit|legacy] [--boottype bootcmd|reset|""] outfile
...
"")
	;;
*)
	help "unknown boottype '$BOOTTYPE'"
	;;
esac
...
make_script()
{
	...
	"")
		cat <<- EOFMKSB >> "$script"
			echo "Either directly boot or perform a reset/powercycle"
			exit 0
		EOFMKSB
		;;
	esac
